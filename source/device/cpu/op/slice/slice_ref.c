...
static int onnx_run(const int8_t* in_data, int8_t** out_data, int element_size, const struct slice_param_ref* param)
{
    ...
    else if (param->dim_num == 5)
    {
        const int* in_dim_new = param->in_shape_5;
        ...
        int start_2 = (param->axis == 2) ? param->begin : 0;
        int start_3 = (param->axis == 3) ? param->begin : 0;
        int start_4 = (param->axis == 4) ? param->begin : 0;
        ...
        if (param->step > 1)
        {
            ...
            int step_2 = (param->axis == 2) ? param->step : 1;
            int step_3 = (param->axis == 3) ? param->step : 1;
            int step_4 = (param->axis == 4) ? param->step : 1;
            for (int n = start_0; n < stop_0; n = n + step_0)
            {
                for (int i = start_1; i < stop_1; i = i + step_1)
                {
                    for (int j = start_2; j < stop_2; j = j + step_2)
                    {
                        for (int k = start_3; k < stop_3; k = k + step_3)
                        {
                            for (int l = start_4; l < stop_4; l = l + step_4)
                            {
                                int input_index = n * in_dim_1 * in_dim_2 * in_dim_3 * in_dim_4 + i * in_dim_2 * in_dim_3 * in_dim_4 + j * in_dim_3 * in_dim_4 + k *in_dim_4 + l;
                                memcpy(output, input + input_index * element_size, element_size);
                                output += element_size;
                            }
                        }
        ...
        for (int n = start_0; n < stop_0; ++n)
        {
            for (int i = start_1; i < stop_1; ++i)
            {
                for (int j = start_2; j < stop_2; ++j)
                {
                    ...
                    {
                        int len = stop_4 - start_4;
                        int input_off = n * in_dim_1 * in_dim_2 * in_dim_3 * in_dim_4 + i * in_dim_2 * in_dim_3 * in_dim_4 + j * in_dim_3 * in_dim_4 + start_3 * in_dim_4 + k;
                        memcpy(output, input + input_off * element_size, (size_t)len * element_size);
                        output += len * element_size;
                    }
                }
...
static int run(struct node_ops* node_ops, struct exec_node* exec_node, struct exec_graph* exec_graph)
{
    ...
    else if (op_param.ismxnet || op_param.isonnx)
    {
        ...
        for (unsigned int idx = 0; idx < input_tensor->dim_num; idx++)
        {
            ...
            else if (input_tensor->dim_num == 5)
            {
                op_param.in_shape_5[idx] = input_tensor->dims[idx];
            }
        }
	...
	for(int i = 0;i<input_tensor->dim_num;i++)
	{
        ...
        {
            if (input_tensor->dims[0] == out_tensor->dims[0] && input_tensor->dims[1] == out_tensor->dims[1] && input_tensor->dims[2] == out_tensor->dims[2])
            {
                memcpy((void*)(out_data_ptrs[0]), (void*)input, mem_size * input_tensor->elem_num);
                sys_free(out_data_ptrs);
                return true;
        ...
        {
            if (input_tensor->dims[0] == out_tensor->dims[0] && input_tensor->dims[1] == out_tensor->dims[1] && input_tensor->dims[2] == out_tensor->dims[2] && input_tensor->dims[3] == out_tensor->dims[3] && input_tensor->dims[4] == out_tensor->dims[4])
            {
                memcpy((void*)(out_data_ptrs[0]), (void*)input, mem_size * input_tensor->elem_num);
                sys_free(out_data_ptrs);
                return true;
