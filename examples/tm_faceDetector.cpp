...
void get_input_data_yolov3(const char* image_file, float* input_data, int img_h, int img_w, const float* mean, const float* scale)
{
    ...
    if (sample.channels() == 3)
        cv::cvtColor(sample, img, cv::COLOR_RGB2GRAY);
    else
	img = sample.clone();
    ...
    for (int h = 0; h < img_h; h++)
    {
        ...
        {
                int in_index = h * img_w  + w ;
                int out_index = h * img_w + w;
                input_data[out_index] = (img_data[in_index] - mean[0]) * scale[0];
        }
    }
}
...
int main(int argc, char* argv[])
{
    ...
    fprintf(stdout, "line:%d start to prerun_graph_multithread\n",__LINE__);
